...
object readFileFromFastqSimple {
  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("readFileFromFastqSimple").setMaster("local")
    ...
    val fq0 = sc.textFile("file/adam/hs38DH/hs38DHSE1L100F1.fq")
    fq0.foreach(println)

    //load Fastq
    val fq1 = ac.loadFastq("file/adam/hs38DH/hs38DHSE1L100F1.fq", None, None, ValidationStringency.STRICT)
    println(fq1.count);
